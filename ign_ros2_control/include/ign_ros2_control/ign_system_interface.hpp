...
namespace ign_ros2_control
{
  ...
  template<class ENUM, class UNDERLYING = typename std::underlying_type<ENUM>::type> class SafeEnum 
  {
    public:
      SafeEnum() : mFlags(0) {}
  ...
  class IgnitionSystemInterface : public hardware_interface::BaseInterface<hardware_interface::SystemInterface>
  {
    public:

      IgnitionSystemInterface(){
        std::cout << "New system interface!" << std::endl;
      }

      /// \brief Configure the system interface
      /// param[in] model_nh Pointer to the ros2 node
      /// param[in] update_rate controller update rate
      virtual void configSim(
